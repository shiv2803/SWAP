...
# BLE is the lowest-priority adaptive protocol in the UNO Q decision policy
# (control spec, "BLE Priority and Post-Connection Guard"): prefer Wi-Fi
# first, LoRa second, and BLE last "unless measured conditions explicitly
# justify BLE". Expressed as a margin, so BLE is only selected when it beats
# the best non-BLE option by more than noise -- a marginal BLE win is not the
# explicit justification the policy asks for. Deliberately larger than
# HYSTERESIS_MARGIN: this is a standing preference, not a flap guard.
BLE_PREFERENCE_MARGIN = 6.0

# Tie-break order when scores are effectively equal, highest priority first.
PROTOCOL_PRIORITY = (PROTOCOL_WIFI, PROTOCOL_LORA, PROTOCOL_BLE)
...
def _apply_ble_priority(candidate: int, raw_scores: Dict[int, float]) -> Tuple[int, bool]:
    """Enforce Wi-Fi > LoRa > BLE unless BLE is clearly the right answer.

    Returns (protocol, deprioritized). `deprioritized` is True only when this
    policy actually overrode a BLE pick, so /status can show that the choice
    was a policy decision rather than a scoring one.
    """
    if candidate != PROTOCOL_BLE:
        return candidate, False
    best_alternative = max(
        (p for p in raw_scores if p != PROTOCOL_BLE),
        key=lambda p: (raw_scores[p], -PROTOCOL_PRIORITY.index(p)),
    )
    if raw_scores[PROTOCOL_BLE] - raw_scores[best_alternative] > BLE_PREFERENCE_MARGIN:
        return PROTOCOL_BLE, False
    return best_alternative, True
...
class LinkQualityModel:
    ...
    def _predict_from_window(self, node_id: str, window: FeatureWindow) -> Optional[Decision]:
        ...
        # --- BLE priority policy ----------------------------------------
        # Applied before hysteresis so the guards downstream operate on the
        # protocol the policy actually allows, not one it would veto.
        candidate, ble_deprioritized = _apply_ble_priority(candidate, raw_scores)
        if ble_deprioritized:
            confidence = probabilities[candidate]
        ...
        return Decision(
            ...
            ble_deprioritized=ble_deprioritized,
